...
`,
  ...
  it.skip('uses split-window launch for explicit run inside tmux while preserving the interview pane', async () => {
    const repo = await initRepo();
...
`,
  ...
  it('uses split-window launch for explicit run inside tmux while preserving the interview pane (platform-agnostic)', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omx-autoresearch-run-split-bin-'));
    try {
      const missionDir = join(repo, 'missions', 'demo');
...
`,
      ...
      const tmuxOutput = await readFile(tmuxLog, 'utf-8');
      const missionPathCandidates = [...new Set([
        missionDir,
        missionDir.replace(/^\/private/, ''),
        missionDir.startsWith('/var/') ? `/tmp/${missionDir.slice('/var/'.length)}` : missionDir,
      ])];
      const missionPathPattern = missionPathCandidates
        .map((candidate) => candidate.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
        .join('|');
      assert.match(tmuxOutput, /split-window -h -t %9 -d -P -F #\{pane_id\} -c/);
      assert.match(tmuxOutput, new RegExp(`'autoresearch' '(?:${missionPathPattern})' '--model' 'gpt-5'`));
      assert.doesNotMatch(tmuxOutput, /kill-pane -t %9/);
    } finally {
      await rm(repo, { recursive: true, force: true });
      await rm(fakeBin, { recursive: true, force: true });
