...
const defaultGitRunner: GitRunner = (command, args, options = {}) =>
...
export function hasUsableNodeModules(repoRoot: string): boolean {
  return REQUIRED_NODE_MODULE_MARKERS.every((marker) =>
    existsSync(join(repoRoot, "node_modules", marker)),
  );
}

export function resolveGitCommonDir(cwd: string, gitRunner: GitRunner): string | null {
...
export function resolveReusableNodeModulesSource(
  repoRoot: string,
...
export function ensureRepoDependencies(
  ...
  const reusableNodeModules = resolveReusableNodeModulesSource(
    repoRoot,
    gitRunner,
  );
  if (reusableNodeModules) {
    symlink(
      reusableNodeModules,
      targetNodeModules,
      platformName === "win32" ? "junction" : "dir",
    );
    log(
      `[smoke:packed-install] Reusing node_modules from ${reusableNodeModules}`,
    );
    return {
      strategy: "symlink",
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const packOutput = JSON.parse(
      (pack.stdout as string).slice((pack.stdout as string).indexOf("[")),
    ...
    run("npm", ["install", "-g", tarballPath, "--prefix", prefixDir], {
      cwd: repoRoot,
    });
