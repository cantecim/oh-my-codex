...
type StringSpawnResult = SpawnSyncReturns<string>;
type ResolveGitCommonDirRunner = Parameters<typeof resolveGitCommonDir>[1];
type ResolveReusableNodeModulesSourceRunner = NonNullable<
  Parameters<typeof resolveReusableNodeModulesSource>[1]
>;
type EnsureRepoDependenciesOptions = NonNullable<
  Parameters<typeof ensureRepoDependencies>[1]
>;
type EnsureRepoDependenciesGitRunner = NonNullable<
  EnsureRepoDependenciesOptions["gitRunner"]
>;
...
test("packed install smoke stays limited to boot + core commands", () => {
  ...
  assert.equal(
    PACKED_INSTALL_SMOKE_CORE_COMMANDS.some(
      (argv) =>
        (argv as readonly string[]).includes("explore") ||
        (argv as readonly string[]).includes("sparkshell"),
    ),
    false,
  );
});
...
test("resolveGitCommonDir resolves relative git common dir output against the repo root", () => {
  const gitRunner: ResolveGitCommonDirRunner = (_command, _args, _options) => ({
    status: 0,
    stdout: "../primary/.git\n",
    stderr: "",
  }) as StringSpawnResult;
  const commonDir = resolveGitCommonDir("/tmp/worktree", gitRunner);
...
test("hasUsableNodeModules requires the packaged build dependencies", async () => {
  ...
  try {
    ...
    await mkdir(join(nodeModules, "@modelcontextprotocol", "sdk"), {
      recursive: true,
    });
    ...
    await writeFile(
      join(nodeModules, "@modelcontextprotocol", "sdk", "package.json"),
      "{}",
    );
...
test("resolveReusableNodeModulesSource reuses primary worktree node_modules when available", async () => {
  ...
  try {
    ...
    await mkdir(join(primaryRepo, "node_modules", "typescript"), {
      recursive: true,
    });
    await mkdir(join(primaryRepo, "node_modules", "@iarna", "toml"), {
      recursive: true,
    });
    await mkdir(
      join(primaryRepo, "node_modules", "@modelcontextprotocol", "sdk"),
      { recursive: true },
    );
    await mkdir(join(primaryRepo, "node_modules", "zod"), { recursive: true });
    await writeFile(
      join(primaryRepo, "node_modules", "typescript", "package.json"),
      "{}",
    );
    await writeFile(
      join(primaryRepo, "node_modules", "@iarna", "toml", "package.json"),
      "{}",
    );
    await writeFile(
      join(
    ...
    await writeFile(
      join(primaryRepo, "node_modules", "zod", "package.json"),
      "{}",
    );
    await mkdir(worktreeRepo, { recursive: true });

    const gitRunner: ResolveReusableNodeModulesSourceRunner = (
      _command,
      _args,
      _options,
    ) => ({
    ...
    const reusable = resolveReusableNodeModulesSource(worktreeRepo, gitRunner);
...
test("ensureRepoDependencies symlinks a reusable primary worktree node_modules", async () => {
  ...
  try {
    ...
    const events: string[] = [];
    const gitRunner: EnsureRepoDependenciesGitRunner = (
      _command,
      _args,
      _options,
    ) => ({
    ...
    const result = ensureRepoDependencies(worktreeRepo, {
      ...
      install: () => {
        throw new Error(
          "install should not be called when a reusable node_modules source exists",
        );
      },
    ...
    assert.equal(
      result.sourceNodeModulesPath,
      join(primaryRepo, "node_modules"),
    );
    assert.equal(
      events[0],
      `[smoke:packed-install] Reusing node_modules from ${join(primaryRepo, "node_modules")}`,
    );
  } finally {
    await rm(root, { recursive: true, force: true });
  }
