...
async function withPrefixedPath<T>(
  dir: string,
  run: () => Promise<T>,
): Promise<T> {
  return await withEnv({ PATH: prefixedPath(dir) }, run);
}
...
describe('scaleUp', () => {
  ...
  it('persists scaled-up task roles in canonical task state and inbox ids', async () => {
    ...
    try {
      ...
      await withPrefixedPath(fakeBinDir, async () => {
...
`,
      ...
      await withPrefixedPath(fakeBinDir, async () => {
        ...
        await initTeamState('rollback-worktree', 'task', 'executor', 1, cwd, undefined, buildIsolatedEnv(), {
...
`,
      ...
      await withPrefixedPath(fakeBinDir, async () => {
        ...
        await writeFile(join(cwd, 'AGENTS.md'), '# Root project instructions\n');
        await initCommittedGitRepo(cwd);
        await initTeamState('canonical-root', 'task', 'executor', 1, cwd, undefined, buildIsolatedEnv(), {
          workspace_mode: 'worktree',
          leader_cwd: cwd,
          team_state_root: join(cwd, '.omx', 'state'),
        });
...
`,
      ...
      await withPrefixedPath(fakeBinDir, async () => {
        ...
        await writeFile(join(cwd, 'AGENTS.md'), '# Root project instructions\n');
        await initCommittedGitRepo(cwd);
        await initTeamState('mini-tuned-root', 'task', 'executor', 1, cwd, undefined, buildIsolatedEnv(), {
          workspace_mode: 'worktree',
          leader_cwd: cwd,
          team_state_root: join(cwd, '.omx', 'state'),
        });
