...
async function writeCompatRuntimeFixture(
  runtimePath: string,
...
describe("team state", () => {
  ...
  it("normalizes legacy manifest policy with dispatch defaults, timeout bounds, and governance split", async () => {
    ...
    try {
      ...
      assert.equal(
        loaded?.governance.cleanup_requires_all_workers_inactive,
        false,
      );
      assert.equal("delegation_only" in (loaded?.policy ?? {}), false);
      assert.equal("nested_teams_allowed" in (loaded?.policy ?? {}), false);
      assert.equal(
        "cleanup_requires_all_workers_inactive" in (loaded?.policy ?? {}),
        false,
      );

      const freshCwd = await mkdtemp(
        join(tmpdir(), "omx-team-manifest-policy-default-"),
      );
      try {
        await initTeamState(
          "team-policy-default",
        ...
        assert.equal(
          fresh?.governance.cleanup_requires_all_workers_inactive,
          true,
        );

        const freshManifestPath = join(
          freshCwd,
          ".omx",
          "state",
          "team",
        ...
        const persisted = JSON.parse(
          await readFile(freshManifestPath, "utf8"),
        ) as {
...
`,
      ...
      await withEnv(
        ...
        async () => {
          ...
          const jsonPayload =
            stateDirFlag > jsonStart
              ? queueLine.slice(jsonStart, stateDirFlag)
              : queueLine.slice(jsonStart);
  ...
  it("dispatch request store enqueues, dedupes, and transitions idempotently", async () => {
    ...
    try {
      ...
      const notified = await markDispatchRequestNotified(
        "team-dispatch",
      ...
      const notifiedAgain = await markDispatchRequestNotified(
        "team-dispatch",
        first.request.request_id,
        {},
        cwd,
      ...
      const delivered = await markDispatchRequestDelivered(
        "team-dispatch",
        first.request.request_id,
        {},
        cwd,
  ...
  it("prefers bridge-authored dispatch records without mutating the legacy requests file", async () => {
    const cwd = await mkdtemp(
      join(tmpdir(), "omx-team-dispatch-bridge-authority-"),
    );
    try {
      await initTeamState(
      ...
      const runtimeLogPath = join(cwd, "runtime.log");
      await mkdir(fakeBinDir, { recursive: true });
      await writeCompatRuntimeFixture(
        join(fakeBinDir, "omx-runtime"),
        runtimeLogPath,
      );
      await withEnv(
        ...
        async () => {
          ...
          const delivered = await readDispatchRequest(
            "team-dispatch-bridge-authority",
            queued.request.request_id,
            cwd,
          );
          ...
          assert.deepEqual(
            JSON.parse(after),
            [],
            "bridge-success path should not rewrite legacy dispatch requests.json",
          );
        },
      );
    } finally {
  ...
  it("dispatch request store allows failed reason patches and fallback recovery to notified", async () => {
    const cwd = await mkdtemp(
      join(tmpdir(), "omx-team-dispatch-store-failed-"),
    );
    try {
      ...
      const recovered = await markDispatchRequestNotified(
        ...
        queued.request.request_id,
        {
          last_reason: "fallback_confirmed:tmux_send_keys_sent",
          failed_at: undefined,
        },
        cwd,
      );
      ...
      assert.equal(
        ...
        "fallback_confirmed:tmux_send_keys_sent",
      );
      ...
      const patched = await transitionDispatchRequest(
        ...
        {
          last_reason:
            "fallback_confirmed_after_failed_receipt:tmux_send_keys_sent",
        },
        cwd,
      );
      ...
      const reread = await readDispatchRequest(
        "team-dispatch-failed",
        queued.request.request_id,
        cwd,
      );
  ...
  it("initTeamState persists workspace metadata to config + manifest", async () => {
    ...
    try {
      ...
      assert.deepEqual(cfg.worktree_mode, {
        enabled: true,
        detached: false,
        name: "feature/team-meta",
      });
      ...
      assert.deepEqual(manifest?.worktree_mode, {
        enabled: true,
        detached: false,
        name: "feature/team-meta",
      });
  ...
  it("resolves task/mailbox/approval paths under explicit OMX_TEAM_STATE_ROOT from a worker cwd (worker-env contamination regression)", async () => {
    ...
    try {
      ...
      await withEnv({ OMX_TEAM_STATE_ROOT: explicitStateRoot }, async () => {
        ...
        const approval = await readTaskApproval(
          "team-explicit-root",
          task.id,
          workerCwd,
        );
        ...
        const explicitTeamRoot = join(
          explicitStateRoot,
          "team",
          "team-explicit-root",
        );
        assert.equal(
          existsSync(join(explicitTeamRoot, "tasks", `task-${task.id}.json`)),
          true,
        );
        assert.equal(
          existsSync(join(explicitTeamRoot, "mailbox", "leader-fixed.json")),
          true,
        );
        assert.equal(
          existsSync(
            join(explicitTeamRoot, "approvals", `task-${task.id}.json`),
          ),
          true,
        );
        assert.equal(
          existsSync(
            join(workerCwd, ".omx", "state", "team", "team-explicit-root"),
          ),
          false,
        );
      });
  ...
  it("claimTask enforces dependency readiness (blocked_dependency)", async () => {
    ...
    try {
      ...
      const dep = await createTask(
        "team-deps",
        { subject: "dep", description: "d", status: "pending" },
        cwd,
      );
  ...
  it("claimTask claim locking yields deterministic claim_conflict", async () => {
    ...
    try {
      ...
      const conflicts = [c1, c2].filter(
        (c) => !c.ok && c.error === "claim_conflict",
      ).length;
      assert.equal(oks, 1);
  ...
  it("claimTask owner write failure cleans up claim lock without orphan lock dir", async () => {
    const cwd = await mkdtemp(
      join(tmpdir(), "omx-team-claim-owner-write-fail-"),
    );
    let previousUmask: number | null = null;
    try {
      await initTeamState("team-owner-write-fail", "t", "executor", 1, cwd);
      const t = await createTask(
        "team-owner-write-fail",
        { subject: "a", description: "d", status: "pending" },
        cwd,
      );
  ...
  it("transitionTaskStatus returns invalid_transition for illegal transition", async () => {
    ...
    try {
      ...
      const bad = await transitionTaskStatus(
        "team-trans",
        t.id,
        "pending",
        "completed",
  ...
  it("transitionTaskStatus rejects non-terminal transitions from in_progress", async () => {
    const cwd = await mkdtemp(
      join(tmpdir(), "omx-team-transition-nonterminal-"),
    );
    try {
      await initTeamState("team-trans-nonterminal", "t", "executor", 1, cwd);
      const t = await createTask(
        "team-trans-nonterminal",
        { subject: "a", description: "d", status: "pending" },
        cwd,
      );
      const claim = await claimTask(
  ...
  it("transitionTaskStatus returns claim_conflict when claim owner diverges from task owner", async () => {
    const cwd = await mkdtemp(
      join(tmpdir(), "omx-team-transition-owner-diverge-"),
    );
    try {
      await initTeamState("team-trans-owner-diverge", "t", "executor", 2, cwd);
      const t = await createTask(
        "team-trans-owner-diverge",
        { subject: "a", description: "d", status: "pending" },
        cwd,
      );
      const claim = await claimTask(
  ...
  it("transitionTaskStatus appends task_completed event when task completes", async () => {
    ...
    try {
      ...
      const tr = await transitionTaskStatus(
        "team-events",
        t.id,
        "in_progress",
        "completed",
  ...
  it("transitionTaskStatus persists terminal result and error payloads", async () => {
    ...
    try {
      ...
      const completedTask = await createTask(
        ...
        { subject: "done", description: "d", status: "pending" },
        cwd,
      );
      const completedClaim = await claimTask(
        "team-transition-payload",
        completedTask.id,
        "worker-1",
        completedTask.version ?? 1,
        cwd,
      );
      ...
      const completedReread = await readTask(
        "team-transition-payload",
        completedTask.id,
        cwd,
      );
      ...
      const failedTask = await createTask(
        "team-transition-payload",
        { subject: "fail", description: "d", status: "pending" },
        cwd,
      );
      const failedClaim = await claimTask(
        "team-transition-payload",
        failedTask.id,
        "worker-1",
        failedTask.version ?? 1,
        cwd,
      );
      ...
      const failedReread = await readTask(
        "team-transition-payload",
        failedTask.id,
        cwd,
      );
  ...
  it("reclaimExpiredTaskClaim reopens an expired in-progress task so another worker can claim it", async () => {
    ...
    try {
      ...
      const reclaimed = await reclaimExpiredTaskClaim(
        "team-reclaim-expired",
        t.id,
        cwd,
      );
  ...
  it("mailbox APIs: DM, broadcast, and mark delivered", async () => {
    ...
    try {
      ...
      const delivered = await markMessageDelivered(
        "team-msg",
        "worker-2",
        dm.message_id,
        cwd,
      ...
      const mailboxDisk = await readFile(
        join(
          cwd,
          ".omx",
          "state",
      ...
      const parsed = JSON.parse(mailboxDisk) as {
        messages: Array<{ delivered_at?: string }>;
      };
      assert.ok(
        parsed.messages.some((m) => typeof m.delivered_at === "string"),
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("prefers bridge-authored mailbox records without mutating the legacy mailbox file", async () => {
    const cwd = await mkdtemp(
      join(tmpdir(), "omx-team-mailbox-bridge-authority-"),
    );
    try {
      await initTeamState(
        "team-mailbox-bridge-authority",
        "t",
        "executor",
        2,
        cwd,
      ...
      await withEnv(
        ...
        async () => {
          ...
          if (existsSync(legacyPath)) {
            const after = JSON.parse(await readFile(legacyPath, "utf8")) as {
              messages: unknown[];
            };
            assert.equal(
              after.messages.length,
              0,
              "bridge-success path should not rewrite legacy mailbox JSON",
            );
          }
        },
      );
  ...
  it("markMessageNotified stores notified_at without forcing delivered_at", async () => {
    ...
    try {
      ...
      const marked = await markMessageNotified(
        "team-msg-notify",
        "worker-2",
        dm.message_id,
        cwd,
  ...
  it("sendDirectMessage reuses identical undelivered messages instead of appending duplicates", async () => {
    ...
    try {
      ...
      const third = await sendDirectMessage(
        "team-msg-dedupe",
        "worker-1",
        "leader-fixed",
        "same-body",
      ...
      const mailboxAfter = await listMailboxMessages(
        "team-msg-dedupe",
        "leader-fixed",
        cwd,
      );
  ...
  it("createTask does not overwrite existing tasks when config next_task_id is missing (legacy)", async () => {
    ...
    try {
      ...
      const cfg = JSON.parse(readFileSync(configPath, "utf8")) as unknown as {
        [key: string]: unknown;
      };
      ...
      const cfg2 = JSON.parse(readFileSync(configPath, "utf8")) as unknown as {
        [key: string]: unknown;
      };
  ...
  it("listTasks returns sorted by ID", async () => {
    ...
    try {
      ...
      await createTask(
        "team-4",
        { subject: "c", description: "d", status: "pending" },
        cwd,
      );
  ...
  it("listTasks reads task files in parallel", async () => {
    ...
    try {
      ...
      for (let i = 0; i < N; i++) {
        await createTask(
          "team-parallel",
          { subject: `task-${i}`, description: "d", status: "pending" },
          cwd,
        );
      }
  ...
  it("listTasks ignores malformed and id-mismatched task payloads", async () => {
    ...
    try {
      ...
      await createTask(
        "team-list-validate",
        { subject: "ok", description: "d", status: "pending" },
        cwd,
      );
  ...
  it("updateTask rejects empty string status and leaves task readable", async () => {
    ...
    try {
      ...
      assert.ok(
        reread,
        "task should still be readable after invalid update was rejected",
      );
      assert.equal(reread?.status, "pending");
    } finally {
      await rm(cwd, { recursive: true, force: true });
  ...
  it("updateTask coerces non-array depends_on to [] so claimTask does not crash", async () => {
    ...
    try {
      ...
      await updateTask(
        "team-upd-bad-deps",
        created.id,
        { depends_on: "not-an-array" as never },
        cwd,
      );
  ...
  it("getTeamSummary aggregates task counts correctly", async () => {
    ...
    try {
      ...
      await createTask(
        "team-11",
        { subject: "ip", description: "d", status: "in_progress" },
        cwd,
      );
      await createTask(
        "team-11",
  ...
  it("resolveDispatchLockTimeoutMs returns default when env not set", () => {
    ...
    assert.equal(
      resolveDispatchLockTimeoutMs({ OMX_DISPATCH_LOCK_TIMEOUT_MS: "" }),
      15_000,
    );
    assert.equal(
      resolveDispatchLockTimeoutMs({
        OMX_DISPATCH_LOCK_TIMEOUT_MS: "not-a-number",
      }),
      15_000,
    );
  });
  ...
  it("resolveDispatchLockTimeoutMs reads from env and clamps to bounds", () => {
    ...
    assert.equal(
      resolveDispatchLockTimeoutMs({ OMX_DISPATCH_LOCK_TIMEOUT_MS: "30000" }),
      30_000,
    );
    // Clamps to minimum
    assert.equal(
      resolveDispatchLockTimeoutMs({ OMX_DISPATCH_LOCK_TIMEOUT_MS: "0" }),
      1_000,
    );
    assert.equal(
      resolveDispatchLockTimeoutMs({ OMX_DISPATCH_LOCK_TIMEOUT_MS: "-500" }),
      1_000,
    );
    // Clamps to maximum
    assert.equal(
      resolveDispatchLockTimeoutMs({ OMX_DISPATCH_LOCK_TIMEOUT_MS: "999999" }),
      120_000,
    );
    // Floors non-integer
    assert.equal(
      resolveDispatchLockTimeoutMs({ OMX_DISPATCH_LOCK_TIMEOUT_MS: "5000.9" }),
      5_000,
    );
  });
      ...
      try {
        await assert.rejects(
          () =>
            enqueueDispatchRequest(
              "team-lock-hint",
              { kind: "inbox", to_worker: "worker-1", trigger_message: "test" },
              cwd,
            ),
          (err: Error) => {
            assert.ok(
              err.message.includes("OMX_DISPATCH_LOCK_TIMEOUT_MS"),
              `Expected hint in error, got: ${err.message}`,
            );
            return true;
          },
        );
