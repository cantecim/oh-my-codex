...
describe("notify-hook team leader nudge", () => {
  ...
  it.skip("sends immediate all-workers-idle nudge for active team (leader context)", async () => {
    await withTempWorkingDir(async (cwd) => {
      const omxDir = join(cwd, ".omx");
      const stateDir = join(omxDir, "state");
      const logsDir = join(omxDir, "logs");
  ...
  it("suggests shutdown when all workers are idle and the current task set is complete", async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(
        tmuxLog,
        /Team idle-shutdown is complete and waiting on leader action/,
      );
      assert.match(
        tmuxLog,
        /Next: decide whether to reconcile\/merge results or gracefully shut down: omx team shutdown/,
      );
      assert.doesNotMatch(tmuxLog, /keep polling/);
    });
  });

  it.skip("suggests reusing the team when follow-up tasks are pending and worker panes are still reusable", async () => {
    await withTempWorkingDir(async (cwd) => {
      const omxDir = join(cwd, ".omx");
      const stateDir = join(omxDir, "state");
      const logsDir = join(omxDir, "logs");
  ...
  it("sends immediate all-workers-idle nudge for active team (leader context) (isolated env)", async () => {
    await withTempWorkingDir(async (cwd) => {
      const omxDir = join(cwd, ".omx");
      const stateDir = join(omxDir, "state");
      const logsDir = join(omxDir, "logs");
  ...
  it("suggests reusing the team when follow-up tasks are pending and worker panes are still reusable (isolated env)", async () => {
    await withTempWorkingDir(async (cwd) => {
      const omxDir = join(cwd, ".omx");
      const stateDir = join(omxDir, "state");
      const logsDir = join(omxDir, "logs");
  ...
  it("suggests launching a new team when follow-up tasks are pending but worker panes are no longer reusable", async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.doesNotMatch(
        tmuxLog,
        /assign the next follow-up task to this idle team/,
      );
    });
  });
  ...
  it("does not inject leader nudge into a shell pane", async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeFile(
        fakeTmuxPath,
        buildTeamLeaderTmux(tmuxLogPath, {
          paneProbes: {
            "%71": {
              paneInMode: "0",
              currentCommand: "zsh",
    ...
    await withTempWorkingDir(async (cwd) => {
      ...
      const nudgeEvent = events.find(
        (entry: { type?: string; reason?: string }) =>
          ...
          entry.reason === "stale_leader_panes_alive",
      );
      assert.ok(
        nudgeEvent,
        "should emit stale_leader_panes_alive event after leader nudge send",
      );
    });
  });
  ...
  it("nudges when team progress is stalled even if timing signals are fresh or missing (fallback threshold)", async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(
        tmuxLog,
        /Team stalled-progress: leader stale, no progress 3m/,
      );
      assert.match(
        tmuxLog,
        /Next: omx team status stalled-progress; read worker messages; unblock\/reassign or shutdown\./,
      );
      assert.doesNotMatch(tmuxLog, /keep polling/);
  ...
  it("nudges when team progress is stalled before the leader becomes stale (fallback threshold)", async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(
        tmuxLog,
        /Team stalled-before-stale: worker panes stalled, no progress 3m/,
      );
      assert.match(
        tmuxLog,
        /Next: omx team status stalled-before-stale; read worker messages; unblock\/reassign or shutdown\./,
      );
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.doesNotMatch(tmuxLog, /leader stale/);
  ...
  it("nudges stalled team after configurable worker turn stall window elapses (primary threshold)", async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(
        tmuxLog,
        /Team worker-turn-stall-threshold: worker panes stalled, no progress 45s/,
      );
    });
  });
  ...
  it("bounds repeated stalled-team nudges before leader stale by cooldown", async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.equal(
        sends.length,
        1,
        "cooldown should keep repeated stalled-team nudges bounded",
      );
      assert.match(
        tmuxLog,
        /Next: omx team status stalled-before-stale-bounded; read worker messages; unblock\/reassign or shutdown\./,
      );
    });
  });
